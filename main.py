...
# Update the raid_times dictionary with the start and end times for each raid
raid_times = {
...
# Modify the get_current_raid function to handle multiple active raids
def get_current_raid(): 
    current_minute = datetime.now().minute
    active_raids = []

    # Print current minute to check the time
    print(f"Current Minute: {current_minute}")

    # Loop through each raid and check if it is active
    for raid, times in raid_times.items():
        print(f"Checking {raid} - Start: {times['start']} End: {times['end']}")  # Debug info
        if times["start"] <= current_minute <= times["end"]:
            active_raids.append(raid)  # Add active raid to the list

    if active_raids:
        print(f"Active Raids: {active_raids}")  # Debug info
        return active_raids, "In Progress"  # Multiple active raids
    else:
        return ["jungle_island"], "Not started"  # Default if no raids are active

@app.get("/current_raid_page")
async def current_raid_page(request: Request):
    current_time = datetime.now().strftime('%H:%M:%S')

    # Debug: Print the current time to check if it's as expected
    print(f"Current time: {current_time}")

    active_raids, raid_status = get_current_raid()  # Get active raids

    # Update raid_status dictionary to reflect active raids
    raid_status_dict = {
        "dedu_island": "Not started",
        "snow_island": "Not started",
        "jungle_island": "Not started"
    ...
    for raid in active_raids: 
        raid_status_dict[raid] = raid_status  # Set status to "In Progress" for active raids
